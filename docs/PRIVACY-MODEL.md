...
### The L1 transaction-history leak - and the fix

**Honest acknowledgement:** L1 hides bidder identity at the **account-data layer** (the `BidCommit` stores `sha256(provider_wallet)` not the wallet itself, so `getProgramAccounts` returns opaque hashes). It does **NOT** by itself hide identity at the **transaction-history layer**: every bid tx (commit_bid_init, delegate_bid, write_bid_chunk, finalize_bid) has a fee payer / signer, that signer's pubkey is in the tx envelope in plaintext, and every blockchain explorer indexes it forever. So a casual Solscan visit to the RFP PDA's transaction history shows the bidder wallets in the "By" column.

This is a fundamental property of Solana - any tx requires at least one signer, that signer is plaintext in the tx envelope, period.

**The fix (Day 7+):** the bid composer offers three payout modes:

1. **Default - main wallet payout, auto-rep.** Provider's main wallet signs everything. The L1 tx-history leak is in full effect - anyone can see the main wallet bid here. Most providers want this (it builds public reputation). Free.
2. **Privacy mode - fresh wallet payout, no rep.** Provider generates a brand-new ephemeral wallet client-side, funds it via Cloak's shielded UTXO pool (deposit → ZK-shielded transfer → relay-paid withdraw), then signs all bid txs with the ephemeral wallet. The on-chain "By" column shows only the fresh wallet, which has no other history and no on-chain link to the provider's main treasury. Reputation does NOT accrue (the fresh wallet IS pseudonymous). Costs ~0.005 SOL per bid for Cloak fees.
3. **Privacy with deferred reputation - fresh wallet now, claim later.** Same as mode 2, but the provider can later call `attest_win` from their main wallet to publicly link the win and credit reputation. The provider chooses **when** (or whether) the link becomes public. Selective, deferred, granular reputation reveal - a primitive other procurement platforms can't offer.

Cloak's shielded pool provides **cryptographic** unlinkability between the provider's main wallet and the ephemeral bid signer (Groth16 ZK proof, not just operational obfuscation). Spike-verified end-to-end on devnet 2026-05-01: a wallet with literally 0 SOL and no on-chain history (`G7dWnk4sDpecqCZRy8Q5y2t86NfourvT4K6MuvHrbhxq`) received 0.04485 SOL with no traceable funding link.

**Privacy set caveat:** the cryptographic property is real, but practical anonymity scales with how many other people are using Cloak's pool concurrently. Devnet pool activity is currently modest (~7 distinct depositors per 30-tx window); mainnet is significantly larger. Demo-honest framing.
...
- `programs/tender/src/instructions/commit_bid_init.rs` - bid init + identity binding (Plain or Hashed)
...
- `programs/tender/src/instructions/select_bid.rs` - winner record + reserve enforcement
- `programs/tender/src/instructions/attest_win.rs` - mode-3 deferred reputation claim
- `apps/web/lib/sdks/magicblock.ts` - dual-connection (base + ER), permission lifecycle
- `apps/web/lib/sdks/cloak.ts` - Cloak shielded ephemeral wallet funding (dynamic-imported)
- `apps/web/lib/sdks/magicblock-payments.ts` - milestone release routing through Private Payments API for cross-RFP unlinkability
- `apps/web/lib/crypto/ecies.ts` + `derive-rfp-keypair.ts` + `derive-provider-keypair.ts` + `derive-utxo-keypair.ts` - application-layer crypto
